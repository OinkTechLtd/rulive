...
LiveM3U — это автоматизированный бот, который ищет рабочие прямые ссылки на потоки телеканалов и автоматически обновляет M3U плейлист. Проект ориентирован на источники, доступные в РФ.

## ✨ Особенности

- 🔍 **Автоматический поиск** — робот самостоятельно ищет потоки из настроенных источников
- ✅ **Проверка работоспособности** — каждый поток проверяется на доступность перед добавлением
...
- 🗂️ **Категоризация** — каналы автоматически сортируются по категориям (Федеральные, Новости, Спорт и др.)
- ⚡ **Многопоточность** — параллельная проверка потоков для максимальной скорости
...
Источники потоков хранятся в файле `config/sources.json`. Вы можете добавить свои источники:
...
│   └── sources.json       # Источники потоков
